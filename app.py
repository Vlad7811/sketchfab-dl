...
(function() {
  ...
  window.__doDownload = function() {
    ...
    if (window.allmodel.length === 0) return 'empty';
    var zip = new JSZip();
    ...
    return 'ok:' + file_name + ':' + window.allmodel.length;
  };

  window.__getStatus = function() {
    return {
      models: window.allmodel.length,
      ready: window.__sf_ready
    };
  };
  ...
  window.drawhookcanvas = function(e, imagemodel) {
    ...
    if (imagemodel) {
      ...
      if(!saveimagecache2[url_image]) saveimagecache2[url_image]={name:imagemodel.attributes.name};
      return obr;
    }
    return e;
  };
  ...
  window.drawhookimg = function(gl, t) {
    ...
    if(ext==='png'||ext==='jpg'||ext==='jpeg') name=saveimagecache2[url].name.replace('.'+ext,'')+'.png';
  ...
  Element.prototype.appendChild = function(node) {
    if (node.tagName==='SCRIPT' && node.src &&
      ...
      var req=new XMLHttpRequest(); req.open('GET',node.src,false); req.send('');
      var js=req.responseText, ret;
      ret=func_renderInto1.exec(js); if(ret){var i=ret.index+ret[0].length;js=js.slice(0,i)+',i'+js.slice(i);}
      ret=func_renderInto2.exec(js); if(ret){var i=ret.index+ret[0].length;js=js.slice(0,i)+',image_data'+js.slice(i);}
      ret=fund_drawArrays.exec(js); if(ret){var i=ret.index+ret[0].length;js=js.slice(0,i)+',window.drawhookimg(t,image_data)'+js.slice(i);}
      ret=func_getResourceImage.exec(js); if(ret){var i=ret.index+ret[0].length;js=js.slice(0,i)+'e=window.drawhookcanvas(e,this._imageModel);'+js.slice(i);}
      ret=func_drawGeometry.exec(js); if(ret){var i=ret.index+ret[1].length;js=js.slice(0,i)+';window.attachbody(t);'+js.slice(i);}
      var s=document.createElement('script'); s.type='text/javascript'; s.text=js;
...
QLineEdit {
    ...
    color: #e0e0f0; padding: 9px 14px; font-size: 13px;
}
...
QPushButton#dlBtn {
    ...
    font-weight: bold; font-size: 14px; padding: 13px 36px; min-width: 200px;
}
...
class MainWindow(QMainWindow):
    ...
    def handle_download(self, download: QWebEngineDownloadRequest):
        ...
        def on_finished():
            if download.isFinished():
                path = os.path.join(self.download_dir, suggested)
                self.hintLabel.setText(f"Готово! Файл: {path}")

        download.isFinishedChanged.connect(on_finished)

    def load_model(self):
        url = self.urlInput.text().strip()
        if not url or "sketchfab.com" not in url:
            self.hintLabel.setText("Введи корректную ссылку Sketchfab")
            return
        self.model_loaded = False
        self.dlBtn.setEnabled(False)
        self.dlBtn.setText("⬇  СКАЧАТЬ")
    ...
    def on_load_finished(self, ok):
        ...
        self.hintLabel.setText("Ожидание захвата геометрии... (объектов: 0)")
    ...
    def poll_status(self):
        if not self.model_loaded:
            return
        self.page.runJavaScript(
            "window.__getStatus ? JSON.stringify(window.__getStatus()) : '{\"models\":0,\"ready\":false}'",
            self.update_status
        )
    ...
    def update_status(self, result):
        try:
            ...
            count = data.get('models', 0)
            ready = data.get('ready', False)
            if count > 0:
                self.hintLabel.setText(
                    f"Захвачено объектов: {count} — {'можно скачивать!' if ready else 'JSZip загружается...'}"
                )
                self.dlBtn.setText(f"⬇  СКАЧАТЬ ({count} объектов)")
            else:
                self.hintLabel.setText("Ожидание захвата геометрии... Прокрути модель в окне")
        except:
            pass
    ...
    def do_download(self):
        ...
        self.page.runJavaScript(
            ...
            lambda r: self.hintLabel.setText(
                "Библиотеки ещё не загружены, подожди..." if str(r) == 'not_ready' else
                "Геометрия не захвачена — подожди загрузки модели" if str(r) == 'empty' else
                "Упаковываю файл..."
            )
        )
