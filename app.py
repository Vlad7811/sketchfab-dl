...
(function() {
  ...
  window.__doDownload = function() {
    ...
    if (!JSZip || !saveAs) { alert('Библиотеки ещё загружаются, подожди...'); return; }
    var zip = new JSZip();
  ...
  window.attachbody = function(obj) {
    if (obj._faked !== true &&
      ...
      window.__sf_modelCount = window.allmodel.length;
    }
  };
  ...
  // Patch appendChild to intercept Sketchfab scripts
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- WebView ---
        profile = QWebEngineProfile("sketchfab", self)
        ...
        self.page = SketchfabPage(profile, self)
    ...
    def on_load_finished(self, ok):
        if not ok:
            self.hintLabel.setText("Ошибка загрузки страницы")
            return

        # Inject script into page
        self.page.runJavaScript(INJECT_JS, lambda result: None)

        self.model_loaded = True
        self.dlBtn.setEnabled(True)
        self.hintLabel.setText("Дождись полной загрузки модели (30–60 сек), затем нажми СКАЧАТЬ")
    ...
    def do_download(self):
        if not self.model_loaded:
            return
        self.hintLabel.setText("Скачивание... сохраняется в папку Загрузок")
        self.page.runJavaScript("window.__doDownload ? window.__doDownload() : 'not_ready'",
            lambda r: self.hintLabel.setText(
                "Файл сохранён в папку Загрузок!" if r and str(r).startswith('ok') else
                "Библиотеки ещё загружаются, подожди немного..."
            )
        )


def main():
    # Required for WebEngine on Windows
